...
/// <summary>
/// Validates prompt mode names. A mode name doubles as its JSON filename
/// (<c>{Name}.json</c>), so it must be a safe Windows filename. Shared by the store and
/// the editor view model so the two can never disagree.
/// </summary>
public static class PromptModeNameRules
{
    ...
    /// <summary>
    /// Validates a prompt mode name (as typed, before trimming).
    /// </summary>
    /// <param name="name">The candidate mode name.</param>
